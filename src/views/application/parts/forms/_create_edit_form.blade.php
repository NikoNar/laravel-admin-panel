@if(isset($application)  && !isset($parent_lang_id))
	{!! Form::model($application, ['route' => ['application-update', $application->id], 'method' => 'PUT', 'enctype' => "multipart/form-data"]) !!}
	{!! Form::hidden('id', $application->id) !!}
@elseif(isset($application) && isset($parent_lang_id) )
	{!! Form::model($application, ['route' => 'application-store', 'enctype' => "multipart/form-data", 'method' => 'POST', 'id' => 'application-store']) !!}
	...
	<div class="col-md-12 no-padding">
		...
		<div class="form-group">
			<div class='input-group'>
				...
				@if(isset($application) &&  null != $application_categories = $application->categories()->get()->pluck('id')->toArray())
					@if(!empty($application_categories))
		    			@include('admin-panel::layouts.parts.categories_dropdown', ['multiple' => 'multiple', 'selected' => $application_categories])
					@else
		...
		<div class="clearfix"></div>
		<a href="#" class="pull-right" id="add-category" data-type="application"><i class="fa fa-plus"></i> Add New Category</a>
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="fileupload fileupload-new" data-provides="fileupload">
			<div class="fileupload-preview thumbnail" style="width: 100%;">
				@if(isset($application) && !empty($application->thumbnail))
		  			<img src="{{$application->thumbnail}}" class="img-responsive" alt="" onerror="imgError(this);" id="thumbnail-image">
				@else
