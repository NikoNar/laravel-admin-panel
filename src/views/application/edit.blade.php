...
@section('content')
	<div class="box">
	    <div class="box-header with-border">
	        ...
	        <a href="{{ route('application-create') }}" class="btn btn-primary btn-flat pull-right ">Add New</a>
	        @if(isset($parent_lang_id) || isset($application) && $application->lang == 'arm')
	        	@if(isset($parent_lang_id))
	        		<a href="{{ route('application-edit', $parent_lang_id) }}" class="btn btn-warning btn-flat pull-right margin-right-15"><i class="fa fa-edit"></i> Translate to English</a>
	        	@else 
	        		<a href="{{ route('application-edit', $application->parent_lang_id) }}" class="btn btn-warning btn-flat pull-right margin-right-15"><i class="fa fa-edit"></i> Translate to English</a>
	        	@endif
	        @else
	        	<a href="{{ route('application-translate', $application->id) }}" class="btn btn-warning btn-flat pull-right margin-right-15"><i class="fa fa-edit"></i> Translate to Armenian</a>
	        @endif
	    </div>
	    <div class="box-body">
	        @include('admin-panel::application.parts.forms._create_edit_form')
	    </div>
	    <!-- /.box-body -->
	</div>
@endsection
@section('script')
	...
	<script>
	  ...
	  // 	@if(isset($application) && !empty($application->description))
			// builderOptions = {!! $application->description !!};
   ...
   //      @endif
		// builderOptions = JSON.parse(builderOptions);
	  	...
	  	// $('body').off('submit', '#application-store').on('submit', '#application-store', function(e){
	  	// 	e.preventDefault();
	  	// 	var form = $(this);
	  	...
	  	// 	    	console.log(data); 
    //                 window.location = "/admin/application/edit/" + data.application_id
	  	// 	    }
	  	// 	});
	  	// });
	  </script>
