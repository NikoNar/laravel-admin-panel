...
@section('content')
	<div class="box">
	    <div class="box-header with-border">
	        ...
	        <a href="{{ route('roles.create') }}" class="btn btn-primary btn-flat pull-right ">Add New</a>
	        @if(isset($parent_lang_id) || isset($role) && $role->lang == 'arm')
	        	@if(isset($parent_lang_id))
	        		<a href="{{ route('roles.edit', $parent_lang_id) }}" class="btn btn-warning btn-flat pull-right margin-right-15"><i class="fa fa-edit"></i> Translate to English</a>
	        	@else 
	        		<a href="{{ route('roles.edit', $role->parent_lang_id) }}" class="btn btn-warning btn-flat pull-right margin-right-15"><i class="fa fa-edit"></i> Translate to English</a>
	        	@endif
	        @else
	        	<!-- <a href="" class="btn btn-warning btn-flat pull-right margin-right-15"><i class="fa fa-edit"></i> Translate to Armenian</a> -->
	        @endif
	    </div>
	    <div class="box-body">
	        @include('admin-panel::roles.parts.forms._create_edit_form')
	    </div>
	    <!-- /.box-body -->
	</div>
@endsection
@section('script')
	...
	<script>
	  ...
	  // 	@if(isset($role) && !empty($role->description))
			// builderOptions = {!! $role->description !!};
   ...
   //      @endif
		// builderOptions = JSON.parse(builderOptions);
	  	...
	  	// $('body').off('submit', '#role-store').on('submit', '#role-store', function(e){
	  	// 	e.preventDefault();
	  	// 	var form = $(this);
	  	...
	  	// 	    	console.log(data); 
    //                 window.location = "/admin/role/edit/" + data.role_id
	  	// 	    }
	  	// 	});
	  	// });
	  </script>
