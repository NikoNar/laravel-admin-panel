...
@section('content')
	<div class="box">
	    <div class="box-header with-border">
	        ...
	        <a href="{{ route("portfolio-create") }}" class="btn btn-primary btn-flat pull-right ">Add New</a>
	        @if(isset($parent_lang_id) || isset($portfolio) && $portfolio->lang == 'arm')
	        	@if(isset($parent_lang_id))
	        		<a href="{{ route('portfolio-edit', $parent_lang_id) }}" class="btn btn-warning btn-flat pull-right margin-right-15"><i class="fa fa-edit"></i> Translate to English</a>
	        	@else
	        		<a href="{{ route('portfolio-edit', $portfolio->parent_lang_id) }}" class="btn btn-warning btn-flat pull-right margin-right-15"><i class="fa fa-edit"></i> Translate to English</a>
	        	@endif
	        @else
	        	<a href="{{ route('portfolio-translate', $portfolio->id) }}" class="btn btn-warning btn-flat pull-right margin-right-15"><i class="fa fa-edit"></i> Translate to Armenian</a>
	        @endif
	    </div>
	    <div class="box-body">
	        @include('admin-panel::portfolio.parts.forms._create_edit_form')
	    </div>
	    <!-- /.box-body -->
	</div>
@endsection
@section('script')
	...
	<script>
	 ...
	 //  	@if(isset($portfolio) && !empty($portfolio->description))
		// 	builderOptions = {!! $portfolio->description !!};
  ...
  //       @endif
	 ...
	 //  	})
	  	...
	  	// $('body').off('submit', '#portfolio-store').on('submit', '#portfolio-store', function(e){
	  	// 	e.preventDefault();
	  	// 	var form = $(this);
	  	...
	  	// 	    	console.log(data); 
    //                 window.location = "/admin/portfolio/edit/" + data.portfolio_id
	  	// 	    }
	  	// 	});
	  	// });
	  </script>
