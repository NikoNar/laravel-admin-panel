...
@section('content')
	<div class="box">
	    <div class="box-header with-border">
	        ...
	        <a href="{{ route("news-create") }}" class="btn btn-primary btn-flat pull-right ">Add New</a>
	        @if(isset($parent_lang_id) || isset($news) && $news->lang == 'arm')
	        	@if(isset($parent_lang_id))
	        		<a href="{{ route("news-edit, [$parent_lang_id]") }}" class="btn btn-warning btn-flat pull-right margin-right-15"><i class="fa fa-edit"></i> Translate to English</a>
	        	@else
	        		<a href="{{ route("news-edit", [$news->parent_lang_id]) }}" class="btn btn-warning btn-flat pull-right margin-right-15"><i class="fa fa-edit"></i> Translate to English</a>
	        	@endif
	        @else
	        	<!-- <a href="{{ route("news-translate", [$news->id]) }}" class="btn btn-warning btn-flat pull-right margin-right-15"><i class="fa fa-edit"></i> Translate to Armenian</a> -->
	        @endif
	    </div>
	    <div class="box-body">
	        @include('admin-panel::news.parts.forms._create_edit_form')
	    </div>
	    <!-- /.box-body -->
	</div>
@endsection
@section('script')
	...
	<script>
	  	...
	  	@if(isset($news) && !empty($news->description))
			builderOptions = {!! $news->description !!};
		...
		// builderOptions = JSON.parse(builderOptions);
	  	...
	  	$('body').off('submit', '#news-store').on('submit', '#news-store', function(e){
	  		e.preventDefault();
	  		var form = $(this);
	  		...
	  		$.ajax({
	  		    ...
	  		    success: function(data){
	  		    	console.log(data); 
                    window.location = "/admin/news/edit/" + data.news_id
	  		    }
	  		});
	  	});
	  </script>
